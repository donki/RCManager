<#
.SYNOPSIS
    Construye el paquete MSIX de sOC Remote Connections Manager para la Microsoft Store.
...
param(
    ...
    [string] $IdentityName = "sOCratic.sOCRemoteConnectionsManager",
    ...
    [string] $DisplayName = "sOC Remote Connections Manager",
    [string] $Version,
    [switch] $Autofirmar
)
...
$proyecto = Join-Path $raiz "sOCRCManager.csproj"
$origenPaquete = Join-Path $raiz "Package"
$trabajo = Join-Path $raiz "bin\msix"
$salida = Join-Path $raiz "bin\sOCRCManager.msix"
...
Copy-Item (Join-Path $publicado "sOCRCManager.exe") $trabajo
...
if ($Autofirmar) {
    ...
    $cert = New-SelfSignedCertificate -Type Custom -Subject $Publisher `
        -KeyUsage DigitalSignature -FriendlyName "Remote Connections Manager (solo pruebas)" `
        -CertStoreLocation "Cert:\CurrentUser\My" `
        -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.3", "2.5.29.19={text}")

    $clave = ConvertTo-SecureString -String "pruebas" -Force -AsPlainText
    $pfx = Join-Path $raiz "bin\pruebas.pfx"
    ...
    Export-Certificate -Cert "Cert:\CurrentUser\My\$($cert.Thumbprint)" `
        -FilePath (Join-Path $raiz "bin\pruebas.cer") | Out-Null
